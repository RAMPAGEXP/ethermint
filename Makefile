...
SIMAPP = ./app
HTTPS_GIT := https://github.com/RAMPAGEXP/ethermint.git
...
docker-build:
	...
	docker create --name ethermint -t -i RAMPAGEXP/ethermint:latest ethermint
...
PACKAGE_NAME:=github.com/RAMPAGEXP/ethermint
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/RAMPAGEXP/ethermint/types"
	godoc -http=:6060
...
test-import:
	go test -run TestImporterTestSuite -v --vet=off github.com/RAMPAGEXP/ethermint/tests/importer
